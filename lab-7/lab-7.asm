...
.data?
	...
	DenominatorDT dt ?
;========================
	public VarAExtrn, VarBExtrn, DenominatorDQ, DenominatorDT, Two
	extrn CalculateDenominator@0:near
;========================
.code
...
MeasureExpression proc index:DWORD
	...
	lea eax, cVar
	mov ebx, offset Two
	mov ecx, offset Epsilon
	call CalculateTan
	cmp eax, 1
	je correctVarC
	;-------------------
	invoke ShowTgIsInvalid, num, VarAExtrn, VarBExtrn, cVar, dVar
	jmp exit
	;-------------------
correctVarC:
	lea edi, dVar
	...
	push dword ptr [edi + 4]
	push dword ptr [edi]
	call CalculateSecondAddition
	;-------------------
	fadd
	fst numerator
	fstp numeratorDT
	;-------------------
	call CalculateDenominator@0
	;------------------- 
	fld DenominatorDT
    fldz    
    fcomp   
	...
	invoke ShowDenominatorIsZeroErrorMessage, num, VarAExtrn, VarBExtrn, cVar, dVar, numerator
	fstp st(0)
	jmp exit
	;-------------------
denominator_not_zero:
	...
	fstp result
	;-------------------
	invoke ShowSuccessMessage, num, VarAExtrn, VarBExtrn, cVar, dVar, numerator, DenominatorDQ, result
	;-------------------
exit:
	mov ebx, reg
	ret
